...
import { 
  Sparkles, Moon, Mic, MicOff, Send, Trash2, User, Plus, 
  ChevronRight, ChevronLeft, Image as ImageIcon, BookOpen, 
  Loader2, Clock, AlertCircle, Volume2, Compass, MessageCircle, 
  Palette, Square, ShieldCheck, Wand2, Stars, Lock, Crown, 
  Layout, Waves, Info, CheckCircle2, ArrowRight, Zap, Users, 
  Lightbulb, BarChart3, PieChart, Share2, Copy, BrainCircuit, 
  ScrollText, RefreshCw, AlertTriangle, RotateCcw, Home, 
  TrendingUp, TrendingDown, Activity, Eye, Layers, Sparkle, Hash, 
  MessageSquareQuote, Calendar, Minus, Sun, CloudRain
} from 'lucide-react';
...
// --- API Calls ---
const generateImage = async (promptText, negativePrompt = "") => {
  ...
  } catch (err) { console.warn("Imagen fallback...", err); }

  // 2. Gemini 2.5 Flash Image (Fallback)
  try {
    const fallbackPrompt = negativePrompt 
      ? `${promptText}. (IMPORTANT: Avoid the following styles or elements: ${negativePrompt})`
      : promptText;
...
// ... OnboardingView, HomeView, RecordDreamView, DreamDetailView (unchanged mostly, but provided for context)
...
function RecordDreamView({ profile, onBack, showToast }) {
  ...
  useEffect(() => { let timer; if (phase === 'analyzing' || phase === 'generating') { timer = setInterval(() => setQuoteIdx(prev => (prev + 1) % SUBCONSCIOUS_QUOTES.length), 7000); } return () => clearInterval(timer); }, [phase]);
  useEffect(() => { const SpeechRec = window.SpeechRecognition || window.webkitSpeechRecognition; if (SpeechRec) { const rec = new SpeechRec(); rec.continuous = true; rec.lang = 'ko-KR'; rec.onresult = (e) => { let text = ''; for (let i = e.resultIndex; i < e.results.length; ++i) { if (e.results[i].isFinal) text += e.results[i][0].transcript; } if (text) setInput(prev => (prev ? prev + ' ' : '') + text); }; rec.onend = () => { if (isRecording) rec.start(); }; recognitionRef.current = rec; } }, [isRecording]);
  const toggleRec = () => { if (!recognitionRef.current) { showToast("이 브라우저에서는 음성 인식을 지원하지 않습니다."); return; } if (isRecording) { recognitionRef.current.stop(); setIsRecording(false); } else { recognitionRef.current.start(); setIsRecording(true); } };
  const handleAnalyze = async () => {
    if (!input.trim() || !auth.currentUser) return;
    if (isRecording) toggleRec();
    setPhase('analyzing');
    const styleData = ART_STYLES.find(s => s.id === currentStyle);
    const systemPrompt = `꿈 분석 전문가 '꿈결'. 1. 분석(제목, 프로이트 해석, 일반 해석). 2. 핵심 상징 키워드 추출: 꿈의 내용에서 가장 중요한 명사형 상징 2-5개를 추출하세요. 상징은 정규화하여 추출하세요 (예: '바다'와 '넓은 바다'는 모두 '바다'로 추출). 3. 5가지 정서 수치 측정: joy(기쁨), peace(평온), sadness(슬픔), vitality(활력), anxiety(불안)를 각 0-100 사이 숫자로 측정하세요. [이미지 가이드]: 사용자가 '한복'을 명시하지 않는 한 절대로 한복을 그리지 마세요. 한국인 인물은 현대 패션으로 묘사하세요. JSON 응답 필수.`;
    try {
      ...
      const dreamRef = await addDoc(collection(db, 'artifacts', appId, 'users', auth.currentUser.uid, 'dreams'), { ...analysis, originalInput: input, artStyle: currentStyle, createdAt: serverTimestamp() });
      ...
      setPhase('complete');
    } catch (err) { console.error(err); setPhase('input'); showToast("분석 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요."); }
  };
  ...
  return (<div className="space-y-10 py-6 animate-in slide-in-from-bottom duration-700"><header className="flex items-center justify-between"><button onClick={onBack} className="p-3 glass-panel rounded-2xl text-indigo-300 flex items-center gap-3 text-[10px] font-bold tracking-widest uppercase transition-all"><ChevronLeft className="w-4 h-4" /> BACK</button><KkumGyeolLogo className="w-12 h-12" /><button onClick={onBack} className="p-3 glass-panel rounded-full text-indigo-300"><Home className="w-5 h-5" /></button></header><div className="glass-panel rounded-[2.5rem] p-6 space-y-6"><div className="flex items-center gap-3 text-purple-400 font-serif"><Palette className="w-5 h-5" /><span className="text-[9px] font-bold uppercase tracking-[0.3em]">Lens Style</span></div><div className="grid grid-cols-3 gap-2">{ART_STYLES.map(style => (<button key={style.id} onClick={() => setCurrentStyle(style.id)} className={`py-3 rounded-xl border text-[9px] font-black transition-all ${currentStyle === style.id ? 'bg-white text-black border-white shadow-lg' : 'bg-white/5 border-white/10 text-indigo-200'}`}>{style.name}</button>))}</div></div><div className="relative"><textarea value={input} onChange={(e) => setInput(e.target.value)} placeholder="어젯밤의 무의식을 들려주세요..." className="w-full h-[400px] glass-panel rounded-[3.5rem] p-10 text-2xl focus:outline-none focus:border-white/10 resize-none font-bold break-keep leading-relaxed text-white shadow-inner" /><div className="absolute bottom-10 right-10 flex gap-4"><button onClick={toggleRec} className={`p-6 rounded-full transition-all shadow-xl ${isRecording ? 'bg-red-500 text-white animate-pulse' : 'glass-panel text-indigo-200'}`}>{isRecording ? <MicOff className="w-6 h-6" /> : <Mic className="w-6 h-6" />}</button><button onClick={handleAnalyze} disabled={!input.trim() || isRecording} className="p-6 bg-white text-black rounded-full shadow-2xl active:scale-90 disabled:opacity-20"><Send className="w-6 h-6" /></button></div></div></div>);
}
...
function DreamDetailView({ profile, dream, onBack, showToast }) {
  ...
  useEffect(() => { if (!dream) return; const fetchScenes = async () => { try { const snap = await getDocs(collection(db, 'artifacts', appId, 'users', auth.currentUser.uid, 'dreams', dream.id, 'scenes')); setScenes(snap.docs.map(doc => ({ id: doc.id, ...doc.data() })).sort((a, b) => a.index - b.index)); } catch (e) { console.error(e); } finally { setLoading(false); } }; fetchScenes(); }, [dream]);
  const handleDelete = async () => { if (!confirm("이 꿈 기록을 삭제하시겠습니까?")) return; await deleteDoc(doc(db, 'artifacts', appId, 'users', auth.currentUser.uid, 'dreams', dream.id)); onBack(); };
...
// --- 리포트 화면 (히스토리 + 긍정/부정 요소 분석) ---
function StatsView({ dreams, onBack }) {
  const [reportIndex, setReportIndex] = useState(0); // 0: Latest batch, 1: Previous batch...
  
  // 배치 계산: reportIndex에 따라 7개씩 슬라이싱
  const currentBatch = dreams.slice(reportIndex * STATS_THRESHOLD, (reportIndex + 1) * STATS_THRESHOLD);
  const nextBatch = dreams.slice((reportIndex + 1) * STATS_THRESHOLD, (reportIndex + 2) * STATS_THRESHOLD); // 비교 대상 (더 과거)
  ...
  return (
    <div className="space-y-12 py-6 animate-in fade-in duration-700 pb-24 px-1">
      ...
      <div className="flex items-center justify-between px-4">
        <button disabled={!hasNextReport} onClick={() => setReportIndex(i => i + 1)} className="p-2 rounded-full glass-panel disabled:opacity-20 text-white"><ChevronLeft className="w-6 h-6" /></button>
        ...
        <button disabled={!hasPrevReport} onClick={() => setReportIndex(i => i - 1)} className="p-2 rounded-full glass-panel disabled:opacity-20 text-white"><ChevronRight className="w-6 h-6" /></button>
      </div>
      ...
      {isLatest && !isCompleteBatch ? (
        <div className="glass-panel p-12 rounded-[3.5rem] border-white/5 space-y-10 flex flex-col items-center justify-center text-center relative overflow-hidden">
          ...
          <div className="space-y-4"><h3 className="text-2xl font-serif text-white italic">리포트 작성 중</h3><p className="text-indigo-200/60 leading-relaxed break-keep text-sm">다음 리포트 발행까지 <span className="text-purple-400 font-bold">{STATS_THRESHOLD - currentBatch.length}개</span>의 꿈 조각이 더 필요합니다.<br/>진행률: <span className="text-white font-bold">{Math.floor(progress)}%</span></p></div>
        </div>
      ) : (
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom duration-1000">
          ...
          <section className="bg-gradient-to-br from-purple-900/20 to-indigo-900/20 p-8 rounded-[3.5rem] border border-white/10 space-y-6 relative overflow-hidden shadow-2xl">
            ...
            <div className="space-y-5">
               ...
               <div className="p-5 bg-[#ffffff05] backdrop-blur-sm rounded-2xl border border-white/5 text-sm text-indigo-100/70 leading-relaxed break-keep flex gap-3"><MessageSquareQuote className="w-10 h-10 text-purple-500/40 shrink-0" /><span>{getComparisonInsight()}</span></div>
            </div>
          </section>

          <div className="grid grid-cols-1 gap-6">
             <div className="glass-panel p-8 rounded-[3rem] space-y-8 flex flex-col items-center bg-white/[0.02]">
                <div className="w-full text-[10px] font-black text-indigo-300 uppercase tracking-[0.2em] flex justify-between items-center"><span>Emotional Map</span></div>
                <div className="relative w-56 h-56 flex items-center justify-center py-4">
                   <svg viewBox="0 0 100 100" className="w-full h-full overflow-visible">
                      ...
                      {hasComparison && (
                        <path d={`M 50 ${10 + (100 - prevStats.emotions.joy) * 0.4} L ${90 - (100 - prevStats.emotions.peace) * 0.4} ${40 - (100 - prevStats.emotions.peace) * 0.1} L ${75 - (100 - prevStats.emotions.vitality) * 0.25} ${85 - (100 - prevStats.emotions.vitality) * 0.1} L ${25 + (100 - prevStats.emotions.anxiety) * 0.25} ${85 - (100 - prevStats.emotions.anxiety) * 0.1} L ${10 + (100 - prevStats.emotions.sadness) * 0.4} ${40 - (100 - prevStats.emotions.sadness) * 0.1} Z`} fill="none" stroke="rgba(255,255,255,0.2)" strokeWidth="1" strokeDasharray="2,2" />
                      )}
                      <path d={`M 50 ${10 + (100 - currentStats.emotions.joy) * 0.4} L ${90 - (100 - currentStats.emotions.peace) * 0.4} ${40 - (100 - currentStats.emotions.peace) * 0.1} L ${75 - (100 - currentStats.emotions.vitality) * 0.25} ${85 - (100 - currentStats.emotions.vitality) * 0.1} L ${25 + (100 - currentStats.emotions.anxiety) * 0.25} ${85 - (100 - currentStats.emotions.anxiety) * 0.1} L ${10 + (100 - currentStats.emotions.sadness) * 0.4} ${40 - (100 - currentStats.emotions.sadness) * 0.1} Z`} fill="rgba(168, 85, 247, 0.25)" stroke="#A855F7" strokeWidth="2" className="animate-pulse" />
                      
                      <g transform="translate(50, 5)"><text fontSize="4" fill="#A855F7" textAnchor="middle" fontWeight="black">기쁨({currentStats.emotions.joy})</text><g transform="translate(0, 5)"><ComparisonIndicator type="joy" /></g></g>
                      <g transform="translate(95, 42)"><text fontSize="4" fill="#6366F1" textAnchor="start">평온({currentStats.emotions.peace})</text><g transform="translate(0, 5)"><ComparisonIndicator type="peace" /></g></g>
                      <g transform="translate(80, 92)"><text fontSize="4" fill="#FBBF24" textAnchor="start">활력({currentStats.emotions.vitality})</text><g transform="translate(0, 5)"><ComparisonIndicator type="vitality" /></g></g>
                      <g transform="translate(20, 92)"><text fontSize="4" fill="#EC4899" textAnchor="end">불안({currentStats.emotions.anxiety})</text><g transform="translate(0, 5)"><ComparisonIndicator type="anxiety" /></g></g>
                      <g transform="translate(5, 42)"><text fontSize="4" fill="#3B82F6" textAnchor="end">슬픔({currentStats.emotions.sadness})</text><g transform="translate(0, 5)"><ComparisonIndicator type="sadness" /></g></g>
                   </svg>
                </div>
             ...
             <div className="grid grid-cols-2 gap-4">
               <div className="glass-panel p-6 rounded-[2.5rem] space-y-4 bg-gradient-to-br from-indigo-500/10 to-blue-500/5">
                  <div className="flex items-center gap-2 text-indigo-300 font-bold uppercase text-[9px] tracking-widest"><Sun className="w-4 h-4" /> Positive Elements</div>
               ...
               <div className="glass-panel p-6 rounded-[2.5rem] space-y-4 bg-gradient-to-br from-purple-500/10 to-pink-500/5">
                  <div className="flex items-center gap-2 text-purple-300 font-bold uppercase text-[9px] tracking-widest"><CloudRain className="w-4 h-4" /> Negative Elements</div>
                  <div className="space-y-2">
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Using simple query + in-memory sort to comply with simple query rule
