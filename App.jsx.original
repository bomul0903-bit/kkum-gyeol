...
const generateImage = async (promptText) => {
  ...
  try {
    const result = await fetchWithRetry(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-image-preview:generateContent?key=${apiKey}`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        contents: [{ parts: [{ text: `Depict this dream: ${promptText}. High art.` }] }],
...
const callGemini = async (prompt, systemInstruction) => {
  const schema = {
    ...
    properties: {
      ...
      symbols: { type: "ARRAY", items: { type: "STRING" }, minItems: 2, maxItems: 5 }, // 상징 추출 필드 추가
      joyLevel: { type: "NUMBER" }, // 리포트를 위한 정량 데이터 추가
...
const KkumGyeolLogo = ({ className = "w-full", color = "#A855F7", secondary = "#6366F1", accent = "#FBBF24" }) => (
  <svg viewBox="0 0 160 120" className={className} fill="none" xmlns="http://www.w3.org/2000/svg">
    <defs><linearGradient id="logoGrad" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor={color} /><stop offset="100%" stopColor={secondary} /></linearGradient></defs>
    <path d="M20 15L22 21L28 23L22 25L20 31L18 25L12 23L18 21L20 15Z" fill={accent} />
    <g transform="translate(10, 40)"><path d="M0 0H35V45" stroke="url(#logoGrad)" strokeWidth="10" strokeLinecap="round" strokeLinejoin="round" /><path d="M45 0H80V45" stroke="url(#logoGrad)" strokeWidth="10" strokeLinecap="round" strokeLinejoin="round" /></g>
    <g transform="translate(105, 40)"><path d="M0 0H45V45" stroke="url(#logoGrad)" strokeWidth="10" strokeLinecap="round" strokeLinejoin="round" /></g>
...
function StatsView({ dreams, onBack }) {
  ...
  // 1. 실시간 데이터 분석 로직 (실제 dreams 배열 기반)
  const symbolsMap = {};
    ...
    <div className="space-y-12 py-6 animate-in fade-in duration-700 pb-24 px-1">
      ...
      {!isUnlocked ? (
        ...
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom duration-1000">
          ...
          <section className="bg-gradient-to-br from-purple-900/20 to-indigo-900/20 p-8 rounded-[3.5rem] border border-white/10 space-y-6 relative overflow-hidden shadow-2xl">
            ...
            <div className="space-y-5">
               <p className="text-2xl md:text-3xl font-serif text-white italic leading-tight break-keep">
                 "당신의 내면은 현재 <span className="text-purple-400 underline underline-offset-8 decoration-purple-500/50">{avgJoy > avgAnxiety ? '새로운 가능성을 향한 긍정적 전이' : '해결되지 않은 불안의 정화'}</span> 단계에 머물러 있습니다."
               </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
             ...
             <div className="grid grid-cols-1 gap-4">
                <div className="glass-panel p-6 rounded-[2.5rem] flex items-center gap-6 group hover:border-white/10 transition-all border-l-4 border-l-indigo-500">
                   <div className="w-14 h-14 bg-indigo-500/20 rounded-2xl flex items-center justify-center text-indigo-400"><TrendingUp className="w-7 h-7" /></div>
                   <div>
                ...
                <div className="glass-panel p-6 rounded-[2.5rem] flex items-center gap-6 group hover:border-white/10 transition-all border-l-4 border-l-pink-500">
                   <div className="w-14 h-14 bg-pink-500/20 rounded-2xl flex items-center justify-center text-pink-400"><Eye className="w-7 h-7" /></div>
                   <div>
                ...
                <div className="glass-panel p-6 rounded-[2.5rem] flex items-center gap-6 group hover:border-white/10 transition-all border-l-4 border-l-yellow-500">
                   <div className="w-14 h-14 bg-yellow-500/20 rounded-2xl flex items-center justify-center text-yellow-400"><Layers className="w-7 h-7" /></div>
                   <div>
          ...
          {/* 3. 실제 데이터 기반 반복 상징 추출 섹션 */}
          <section className="glass-panel p-8 rounded-[3.5rem] space-y-8 bg-[#ffffff02]">
             <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 text-indigo-400 font-bold uppercase text-[10px] tracking-[0.2em]"><Layers className="w-5 h-5" /> 반복되는 꿈의 파편들</div>
          ...
          <section className="glass-panel p-8 rounded-[3.5rem] space-y-6 border border-white/10 bg-gradient-to-b from-transparent to-purple-900/5">
             <div className="flex items-center gap-3 text-yellow-400 font-bold uppercase text-[10px] tracking-[0.2em]"><Palette className="w-5 h-5" /> Subconscious Art Preference</div>
          ...
          <div className="text-center space-y-3 pt-12 pb-10">
             ...
             <div className="flex justify-center gap-1 opacity-20"><Sparkle className="w-2 h-2" /><Sparkle className="w-2 h-2 text-purple-400" /><Sparkle className="w-2 h-2" /></div>
             <p className="text-indigo-200/10 text-[9px] break-keep px-10 leading-relaxed">이 리포트는 AI와 심리학적 알고리즘의 결합으로 생성되었습니다. 결과는 당신의 정서적 성장을 돕기 위한 보조 도구로 사용해 주세요.</p>
          </div>
        </div>
      )}
    </div>
